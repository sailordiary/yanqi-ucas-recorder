...
def send_msg(sock, msgtype, msg):
    ...
    nblen = len(msg)
    header = struct.pack('>h', nblen + 8) + struct.pack('hhh', MSG_VER, msgtype, 0)
    msg = header + msg
    sock.sendall(msg)
...
def parse_hdb_frame(data):
    packet = data[48:]
    # HDB frame head
    idx, ntimetick, nframelength, _, nframerate, nwidth, nheight, _, dwsegment, dwflags, dwpacketnumber, nothers = struct.unpack('iiiiiiiiiiii', data[:48])
    # Invalid data.
    if idx < 0 or idx >= 4:
        return {'ret': -1}
    codec = data[12: 16].decode()
    # if codec == 'ADTS': print (nframelength, len(packet))
    res = {'ret': 0, 'header': {'codec': codec, 'idx': idx, 'flen': nframelength, 'fps': nframerate, 'w': nwidth, 'h': nheight, 'seg': dwsegment, 'flags': dwflags, 'tick': ntimetick, 'pktno': dwpacketnumber, 'others': nothers}, 'data': packet}
    ...
    try:
        while True:
            ...
            if len(data) >= 48:
                ...
                if res['ret'] != 0: continue
                if res['header']['codec'] == 'H264':
                    screen_idx = res['header']['idx']
                    if screen_idx == 0:
                        n_vid_packets += 1
                        if n_vid_packets % 100 == 0:
                            print ('Status: {} frames, {} fps, seq={}'.format(n_vid_packets, res['header']['fps'], res['header']['tick']))
